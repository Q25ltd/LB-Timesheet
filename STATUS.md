...
root `npm run check` (typecheck, lint, 194 tests), so it cannot rot unnoticed.
...
| Start shift (date, time, driver, truck, trailer) | 🔶 — **step 1 of the mobile workflow only**. `app/(app)/start-shift.tsx` + `src/screens/StartShiftScreen.tsx`: Home's Start Shift opens it in one tap, and it asks WORKING FOR as a single-selection list — Personal (always the default, never pre-empted by holding one membership) plus each ACTIVE membership by name. Exactly one choice is ever selected. **It is local intent, not authority**: opening the screen and choosing a company produce ZERO network requests, so no tenant token is minted, no server Shift is created and the company is told nothing (D28). The route sits under the `(app)` gate as a STACK sibling of the whole tab group, so it shows no tab bar and the driver cannot leave a part-filled workflow by tapping a tab — see the app-shell row. `Continue` is present and DISABLED because step 2 does not exist. 23 tests; five mutations each failed only their own cases (default flipped to a company, selection made additive, a switch-company call added, Home's button re-disabled, Continue enabled). **Still unbuilt:** start time and the ±15-minute confirmation (D20), vehicle/trailer, checks, defects, fuel, mileage, finish, local persistence, offline queue — and the selection lives in React state only, so it does not survive leaving the screen |
| Company selection (only when driver holds >1 membership) | 🔶 — the SERVER endpoint exists and is proven (15 DB tests). A client call exists — `switchCompany` in `mobile/src/api/account.ts` and `chooseCompany` in `AuthContext` — but it is **NOT proven and has no caller**: no mobile test exercises it and no screen invokes it. There is no picker SCREEN yet, and nothing can create a second membership to pick from |
| Truck check | 🔲 |
| Trailer check | 🔲 |
