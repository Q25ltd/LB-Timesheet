...
| Active Shift — **Discard Shift** | 🔲 — **not rendered at all**, deliberately: a control that opens nothing is worse than an absent one. A driver who starts a shift by mistake still cannot abandon it, which is a real gap and the recommended next increment. Recommended home: a top-right overflow in the header, where Home already places its identity badge |
| Truck check | 🔲 |
| Trailer check | 🔲 |
