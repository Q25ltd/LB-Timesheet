...
root `npm run check` (typecheck, lint, 138 tests), so it cannot rot unnoticed.
...
| **Home / Today** | ✅ — the authenticated landing screen (`mobile/src/screens/HomeScreen.tsx`, wired by `app/(app)/today.tsx`). Brand lockup, a non-interactive identity badge showing the driver's initials, a device-local greeting and date, one prominent shift card, and Sign out. The biometric opt-in offer is unchanged. **It makes ZERO network requests**, which is the property the rest follows from: it cannot know shift or history state, so it claims none. The mock-up's "No active shift" is deliberately NOT rendered — proving it needs a tenant token, which is memory-only and gone after every restore, so checking it would force a company choice onto Home (deferred to Start Shift). No Recent Timesheets section (no history source exists), no company named or selectable (D12 privacy), and no bottom navigation (three of its four destinations do not exist). **Start Shift is rendered and DISABLED**, with no press handler — the next increment switches on that same button. 14 tests; the greeting is proven against two different accounts, so a hard-coded name fails |
| Authenticated route-group gate | ✅ — `app/(app)/_layout.tsx` guards every screen under `(app)`: it HOLDS on `restoring`, renders the routes when authenticated, and redirects to `/sign-in` otherwise. Structural rather than per-screen, so a future authenticated screen is protected without its author remembering (the same polarity as F-10's default-deny route hook). Closes the deep-link dead end — `lbtimesheets://today` on a signed-out phone previously reached a "Signing you in…" placeholder that never signed anyone in and never navigated. 5 tests; deleting the redirect fails three of them and deleting the hold fails the fourth |
