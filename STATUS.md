...
| Active Shift | 🔶 — a minimal destination only (`src/screens/ActiveShiftScreen.tsx`). It proves the handoff and distinguishes the two real states: a shift running with no vehicle, and one running with a vehicle (class, plate, start mileage). **No operational actions exist** — no checks, no Add Vehicle, no Change Vehicle, no fuel, no trailer, no Finish Shift |
| Truck check | 🔲 |
| Trailer check | 🔲 |
