...
calendar date it started, in the company's IANA timezone. What stands between
that decision and a correctly persisted `shiftDate` is missing implementation,
not a missing decision: `Company` carries **no timezone field**, so no company
timezone authority exists in the schema to derive the local date from. *Public
...
| First protected business route | 🔲 — **no open decision blocks it any more.** P1.2b (F-14) closed `2c85f4f`, and O8 closed 2026-08-31 as **D18**: a shift is one timesheet, filed under the local calendar date it started, derived at creation from the start instant in the **company's** IANA timezone and immutable afterwards. What is missing is implementation, and the gap is concrete: (1) `Company` in `api/prisma/schema.prisma` has **no IANA timezone field**, so the authority D18 derives from does not exist — the local date cannot be computed correctly today (a local 00:30 BST start falls on the previous UTC date); (2) the `shiftDate` comment in the schema still calls the rule "(O8, provisional)" and does not mention a timezone; (3) `shiftDate` immutability is design intent — no database constraint prevents an update; (4) no Night Out field exists on `Shift`. Also relevant before this route lands: **F-24**, whose two user-binding checks are still untested, and which FINDINGS.md flags as ideally fixed before the first protected business route relies on the authentication boundary |
