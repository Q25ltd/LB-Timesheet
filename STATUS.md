...
root `npm run check` (typecheck, lint, 239 tests), so it cannot rot unnoticed.
**No SQLite yet** (D25). The open shift is one JSON document in the app's documents directory via `expo-file-system` — durable across process restarts and needing no network. A database bought for a single row would be infrastructure the requirements do not justify (AGENT_WORKFLOW §25); revisit when segments, checks and defects give it something to hold.
...
| Signed-in state with ZERO companies | ✅ — after registering, the driver enters the authenticated app. No join-company gate, no error, no fake membership. Proven by a live end-to-end run that created 0 Company and 0 CompanyMembership rows, and by Home's own suite, which renders identically for a driver with zero memberships and one with a company |
| **Home / Today** | ✅ — the authenticated landing screen (`mobile/src/screens/HomeScreen.tsx`, wired by `app/(app)/today.tsx`). The owner-approved composition: brand lockup, an identity badge of the driver's initials that opens Settings, a device-local greeting and date, one prominent Start Shift card whose copy sits OVER the Home photograph (the truck enters from the right; the asset carries its own left-to-right fade to transparency, so no scrim dulls the photo and no gradient dependency was added), and an honest Recent Timesheets section. **It makes ZERO network requests**, which is the property the rest follows from: it cannot know shift or history state, so it claims none. The mock-up's "No active shift" is deliberately NOT rendered — proving it needs a tenant token, which is memory-only and gone after every restore, so checking it would force a company choice onto Home (deferred to Start Shift). No company named or selectable (D12 privacy). **Start Shift is LIVE** and opens the workflow in one tap — no picker, no dialog, no request. The card's photograph and its crop ratio live in `src/screens/homeCardImage.ts`, so replacing the asset is one line and touches no layout or test. **Reviewed on an iPhone 17 Pro simulator by the owner**, twice: the first pass was rejected for a drawn truck glyph that did not read as a truck, an auth-hero photograph whose warm sunrise fought Home's cool surface, and a composition too tall to fit without scrolling. All three corrected. 16 tests; the greeting is proven against two different accounts, so a hard-coded name fails |
| Authenticated route-group gate | ✅ — `app/(app)/_layout.tsx` guards every screen under `(app)`: it HOLDS on `restoring`, renders the tab shell when authenticated, and redirects to `/sign-in` otherwise. The check sits ABOVE the navigator, so all four destinations are covered once and a screen added later inherits it without its author remembering (the same polarity as F-10's default-deny route hook). Closes the deep-link dead end — `lbtimesheets://today` on a signed-out phone previously reached a "Signing you in…" placeholder that never signed anyone in and never navigated. 5 tests; deleting the redirect fails three of them and deleting the hold fails the fourth |
| **Authenticated app shell — four destinations** | ✅ — the gate renders a `Stack`; the four everyday destinations live in a nested `(tabs)` group (`app/(app)/(tabs)/`) with a custom bar (`src/navigation/AppTabBar.tsx`) so the design is ours and the routing is the navigator's. Operational workflows — Start Shift, Active Shift and Add Vehicle today — are Stack SIBLINGS of that group rather than members of it, which is what makes them tab-bar-free structurally rather than by hiding a bar. `appNavigationStructure.test.tsx` asserts the shape by recording what each navigator registers, so moving a workflow back among the tabs turns it red. `src/navigation/tabs.ts` is the single source of truth both read, so a control cannot point at a destination the app lacks. Home · Timesheets · My Records · Settings, Home first and default. Icons are drawn from React Native views (`src/components/TabIcon.tsx`) — **no icon dependency**: `@expo/vector-icons` would ship megabytes of fonts for four glyphs and `expo-symbols` is iOS-only. 8 tests; a bar that routed every control to one destination, or hard-coded the active tab, each fails its own case. **Navigation verified on the simulator**, not only in tests |
| **Timesheets / My Records landing pages** | ✅ (as landing states) / 🔲 (as features) — real, titled, designed pages that say plainly the feature is being built, sharing one `AppScreen` frame and one `ComingSoonCard`. **Neither fabricates the data it is a placeholder for**: proven by asserting no rendered NUMBER appears on either page, since a specimen row or statistic must put a date, time, distance or total on screen. Timesheets makes no request (no history endpoint exists); My Records reads no local store, and a test pins that no SQLite dependency was added (D25). 8 tests |
| **Settings** | ✅ — real capability only, and the screen where existing capability finally became reachable. The authenticated name and email from the session; the biometric preference through `AuthProvider`'s existing `enableBiometricUnlock`/`disableBiometricUnlock` (D26 unchanged) — a device with no enrolment gets an honest "can't be used here" and NO switch; and Sign out. **Sign out moved here from Home** and exists in exactly one place: semantics unchanged — server revocation first while the token is valid, local clear either way, biometric opt-in cleared too. Deliberately absent, because none exists: change password, email verification, delete account, notifications, subscription, company management. 9 tests; rendering a literal account instead of the session's fails |
...
| Session restore after app restart | ✅ — at startup the app reads the SecureStore secret, applies the biometric gate when opted in, redeems it through `/auth/refresh`, **replaces the rotated secret**, reads `/auth/me`, and only then becomes authenticated. `AuthProvider` has an explicit `restoring` status and both `app/index.tsx` and the `(app)` gate HOLD on it, so there is no sign-in flash. A refused credential is deleted; an OFFLINE failure KEEPS it, because a tunnel is not a logout. Company authority is never restored — a tenant token is never stored |
| Start shift (date, time, driver, truck, trailer) | 🔶 — **steps 1 and 2 of the mobile workflow**. `app/(app)/start-shift.tsx` + `src/screens/StartShiftScreen.tsx`, a Stack sibling of the tab group so the workflow has no tab bar. Home's Start Shift opens it in one tap and it asks exactly four things: WORKING FOR (Personal always the default, never pre-empted by holding one membership; plus each ACTIVE membership by name, single selection), START TIME (defaulted ONCE from the device clock, freely editable, and the edited value is what persists), DO YOU HAVE A VEHICLE? (Yes / **Not yet** — never "No") and, only on Yes, vehicle class, number plate and start mileage. **A vehicle is optional (D29):** "Not yet" is a complete answer and the shift goes active without one. Plate is trimmed and upper-cased, never format-validated (plates are international); mileage must be a whole non-negative number and is never defaulted. Switching Yes → Not yet carries NO vehicle into the shift. **Pressing Start Shift writes a LOCAL record and makes no request at all** (D28) — verified on device with the API stopped. `Continue` is gone; the action is Start Shift and it stays unavailable until the branch is valid. 54 tests across steps 1 and 2; mutations proving one-open-shift, persistence, the hidden-field leak, mileage validation, network-independence and the keyboard-safe scrolling contract each failed only their own cases. **The keyboard no longer covers Number plate or Start mileage** (`164cba8`): the form's scroll view declares `automaticallyAdjustKeyboardInsets`, `keyboardDismissMode="on-drag"` and `keyboardShouldPersistTaps="handled"` — no measured keyboard heights, no device offsets, no permanent padding. Jest has neither a keyboard nor geometry, so the three added cases guard that structure; **the behaviour itself was verified in real pixels on the owner's physical iPhone** — both fields visible while typing, numeric pad for mileage, scrolling with the keyboard open, drag dismissal, and correct layout after dismissal. **Still unbuilt:** the ±15-minute confirmation (D20), trailer, checks, defects, fuel, mileage capture beyond the start reading, finish, and any server synchronisation |
| Local open shift (offline) | ✅ — `src/shift/localShift.ts`. One JSON document (`logisticbay-open-shift.json`) in the app's documents directory holding the day's local id, working-for context, declared start, optional vehicle and open status — and nothing speculative. Durable across a process restart, proven on device by killing the app and recovering the open shift. **One open shift at a time**, enforced by an idempotent start: a second press, a retry or a relaunch returns the day already open rather than creating or overwriting one. **Every write is serialised** — starting a day and adding a vehicle queue behind one another — so a double tap cannot produce two ids or two vehicles. **A stored vehicle records when its use began, in one field, `vehicle.startedAt`** (named for `ShiftSegment.startedAt`): the shift's declared start for a vehicle given at Start Shift, the moment of the add for one added later. It is the one fact a later vehicle history needs that cannot be recovered afterwards; no end mileage, end time or segment list exists. **Backward compatible:** days saved before the field existed hold a Start Shift vehicle with no `startedAt`, and those still load — the value is derived as the shift's `startedAt`, which is exact rather than a guess because Start Shift was then the only way a day got a vehicle — without rewriting the file; a PRESENT but invalid value is refused like any malformed field. An unreadable or wrong-shaped file reports NO open shift rather than crashing or inventing defaults, and is left in place rather than destroyed. 28 tests; mutations requiring the field on read (old days would vanish) and stamping a Start Shift vehicle with the clock each failed their own cases |
| **Active Shift — the driver's workspace** | 🔶 — the screen COMPOSITION is built and owner-approved; the operational actions in it are NOT (see the rows below). `src/screens/ActiveShiftScreen.tsx`, a Stack sibling of the tab group, so it carries no tab bar and is not a fifth destination. Renders `LocalShift` and nothing else: "Shift active", the persisted declared start, and Working For — Personal as itself, a company by its **full name, wrapped over as many lines as it needs and never truncated**, because the name is identifying information and a prefix identifies nothing (owner correction; the two facts are separate columns of one row, so a long name cannot push or overlap the start time). Two real branches: **no vehicle** → "No active vehicle" with Add Vehicle as the obvious next action, and **vehicle** → the plate as the largest element on the screen, class, start mileage, and `Vehicle checks — Not completed`, which is the only check state that exists. Class 1 is a **UNIT** (CURRENT UNIT / Change Unit); Class 2 and van stay "vehicle", because a van driver does not call one a unit. **Unit and trailer are separate assets with separate check workflows** — no combined control exists or ever will. **No trailer is rendered at all:** Class 1 pulls one, none has ever been recorded, and "Trailer: None" would state a fact nobody established. Rank comes from size, position and grouping rather than colour — no traffic-light styling. **Zero network:** rendering the screen and touching every control makes no request. 44 tests, several asserting the ABSENCE of fabricated data across the whole rendered tree (trailer, current/end mileage, fuel or AdBlue totals, driving/POA/break/pay statistics, completed checks, internal ids); two ephemeral mutations — fabricating `Trailer: None`, and wiring Vehicle Checks — each failed only their own cases and were restored byte-identically. **Inspected in real pixels on a 390pt narrow phone and a 402pt phone**, both branches, plus Class 1 with a long international plate, a single-digit mileage and a 48-character company name. Owner-approved after one correction (the company name must not truncate). **Two actions on this screen are live — Discard Shift and Add Vehicle** (their own rows below); everything else remains rendered and inert. The route re-reads the day on every return to the screen, so a vehicle added on top of it shows immediately |
| Active Shift — **Vehicle Checks** functionality | 🔲 — the control is rendered and DISABLED on the workspace above. No checklist, no OK/N-A/Fault, no descriptions, no photos, no signature, and no dedicated screen. A visible disabled control is not an implemented feature |
| Active Shift — **Trailer Checks** functionality | 🔲 — not rendered at all, because no trailer exists to check. A separate screen and a check state independent of the unit's, when trailers land |
| Active Shift — **Add Vehicle** | ✅ — the first vehicle, into a day that started without one (D29). Offered only in the no-vehicle branch; `app/(app)/add-vehicle.tsx` + `src/screens/AddVehicleScreen.tsx`, a Stack sibling of the tabs, so no tab bar. It asks exactly what Start Shift asks — vehicle class (Class 1 / Class 2 / Van), number plate, start mileage — through the SAME fields and rules, now shared in `src/screens/vehicleForm.tsx` so there is one definition of a valid mileage and of how a plate is stored: plate trimmed and upper-cased, never format-validated; mileage a whole number ≥ 0. The button stays unavailable until all three are valid. **Adding is local and makes no request** (D28). It writes the vehicle into the open day and changes nothing else — id, declared start, Working For, status and creation time are carried across unchanged — with `vehicle.startedAt` set to the moment of the add. **Add is not Change:** a day that already has a vehicle is returned untouched, so a stray or repeated Add can never replace one. **One submission however many taps** (a synchronous in-flight guard plus the serialised store); an invalid vehicle is refused rather than written, because a malformed vehicle would make the whole day read as no open shift. On success it returns to Active Shift, which shows the vehicle immediately with the approved Unit/Vehicle wording and **Vehicle checks still Not completed** — adding checks nothing. Reached with no open day it goes to Home; for a day that already has a vehicle, back to Active Shift. The keyboard-safe scroll contract Start Shift was verified with is reused. 28 tests in `addVehicle.test.tsx` plus the store's; mutations allowing Add to overwrite, removing the double-tap guard, and reading the day only once instead of on focus each failed their own cases. **Driven with real touches on the 390pt (Class 2) and 402pt (Class 1) phones:** Number plate stayed visible while typing on the software keyboard, Start mileage opened the number pad and scrolled above it, drag dismissed the keyboard, a deliberate double-tap stored one vehicle, and the vehicle survived killing and relaunching the app. That device run preceded the final field name — the vehicle's start time was then called `addedAt` and set only on added vehicles; the single `vehicle.startedAt` model that replaced it was verified through the store and tests, not re-run on a device |
| Active Shift — **Change Unit / Change Vehicle** functionality | 🔲 — rendered and DISABLED in the vehicle branch only. No mileage prompt, no new registration, no check reset, no persistence change |
| Active Shift — **trailer** functionality | 🔲 — no trailer is captured, stored or displayed anywhere. The workspace leaves a CURRENT TRAILER block directly below the unit card so it can be added without redesigning the screen |
| Active Shift — **Fuel** functionality | 🔲 — rendered and DISABLED. No entry screen, no persistence, and no totals shown, because no fuel record exists. How fuel is modelled is still **O6**, unanswered |
| Active Shift — **AdBlue** functionality | 🔲 — rendered and DISABLED. Same as Fuel, and same open question (**O6**) |
| Active Shift — **Finish Shift** | 🔲 — rendered and DISABLED at the bottom, below a rule and anchored to the foot of the viewport so it is hard to hit while reaching for something else. Persists no finish time, final mileage, night out, notes, totals or submission state, and navigates nowhere |
| Active Shift — **Discard Shift** | ✅ — a driver who books on by mistake can now abandon the day, which closes the one-way door Start Shift used to create: a shift could be started and never ended, because Finish does not exist yet. A quiet **Discard** sits top-right in the header, deliberately as far from Finish Shift as the screen allows — one files the driver's work and the other destroys it, and a cold thumb reaching for the bottom of the screen must not be able to find the wrong one. **One press discards nothing.** It raises a confirmation naming the shift by its start time and saying plainly that it cannot be undone, with `Keep shift` as the safe choice and a platform-destructive `Discard` as the dangerous one — this is the only irreversible action in the app, and the record is local, so there is no copy to recover it from. On confirmation the route deletes the local document and returns to the tabs; **navigation waits for the delete**, so a failure leaves the shift untouched and says so rather than moving the driver somewhere that implies it worked. No request is made (D28 — the day was never sent, so there is nothing to withdraw). Afterwards Start Shift opens the form again rather than continuing the discarded day. **Verified end to end in real pixels:** cancel left the stored shift byte-for-byte intact, confirm deleted it and landed on Home, and Start Shift then offered a fresh form |
| Truck check | 🔲 |
| Trailer check | 🔲 |
